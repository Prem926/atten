...
def add_shift(name, start_time, end_time):
    ...
    cursor.execute("SELECT COUNT(*) FROM shifts WHERE name = ?", (name,))
    exists = cursor.fetchone()[0]
    
    if exists:
        return False  # Shift already exists, return False
        ...
        with col1:
            ...
            with st.form("add_shift"):
                ...
                if st.form_submit_button("Add Shift"):
                    if add_shift(shift_name, start_time.strftime("%H:%M"), end_time.strftime("%H:%M")):
                        st.success("Shift added successfully!")
                    else:
                        st.error("Shift already exists!")
        ...
        with col2:
            ...
            if shifts:
                for idx, shift in enumerate(shifts):
                    with st.expander(f"Shift: {shift[0]} (Start: {shift[1]}, End: {shift[2]})"):
                ...
                st.write("No shifts available. Please create a shift.")
